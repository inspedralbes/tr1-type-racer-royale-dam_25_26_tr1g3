<template>
  <v-app>
    ...
    >
      <v-container
        class="elevation-8 rounded-xl pa-6 bg-white"
        ...
        <v-row>
          ...
          >
            ...
            <div class="mt-4 d-flex gap-2">
              <v-btn color="deep-purple-accent-4" @click="startCamera">
                Abrir cámara
              </v-btn>
              <v-btn
                variant="outlined"
            ...
            <v-card
              class="mt-4 pa-3 text-center rounded-xl"
          ...
          <!-- Columna derecha: ejemplo del ejercicio -->
          <v-col
            cols="12"
            md="6"
          ...
          >
            <h2 class="mb-4 text-primary font-weight-bold">
              Ejemplo de {{ ejercicioLabel }}
            </h2>

            <v-card
              class="overflow-hidden rounded-xl pa-2"
              elevation="6"
              width="100%"
            >
              <img
            ...
            <p class="mt-4 text-grey-darken-1">
              Realiza {{ ejercicioLabel.toLowerCase() }} siguiendo el movimiento
              del ejemplo.<br />
...
// 📌 Obtenemos el parámetro de la ruta (nombre del ejercicio)
const route = useRoute()
const ejercicio = route.params.ejercicio

// 📌 Diccionario de nombres para mostrar
const nombres = {
  flexiones: 'Flexiones',
...
async function detectPose() {
  ...
  async function poseDetectionFrame() {
    ...
    if (poses.length > 0) {
      ...
      checkMovimiento(poses[0]) // 👈 Llama a función genérica
    }
...
// ---------------------------------------------------------
// 🔢 Lógica de conteo (por ahora solo abdominales)
// ---------------------------------------------------------
function checkMovimiento(pose) {
  if (ejercicio === 'abdominales') {
    checkAbdominal(pose)
  }
  // Aquí podrías agregar otras funciones en el futuro:
  // if (ejercicio === 'flexiones') checkFlexiones(pose)
  // if (ejercicio === 'sentadillas') checkSentadillas(pose)
}
